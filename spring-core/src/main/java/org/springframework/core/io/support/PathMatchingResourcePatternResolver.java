...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 如果未指定 ResourceLoader，就调用 {@link DefaultResourceLoader#getResource(String)}
	 */
	@Override
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			// war: 开头的协议，截取 */，非 war: 开头的协议，截取 : 然后判断是否包含通配符
	...
	/**
	 * 包含前缀 classpath*: 但 URL 不包含通配符的处理方法，使用 {@link ClassLoader#getResource(String)}
	 * <p>
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			// hook 方法，默认实现为直接返回参数对象资源
			rootDirResource = resolveRootDirResource(rootDirResource);
			URL rootDirUrl = rootDirResource.getURL();

			// Equinox OSGi 相关，bundle 前缀协议时，反射调用 equinoxResolveMethod
	...
	protected String determineRootDir(String location) {
		...
		while (rootDirEnd > prefixEnd) {
			boolean pattern = getPathMatcher().isPattern(location.substring(prefixEnd, rootDirEnd));
			if (!pattern) break;
			// 定位根目录中最后一个 / 位置（-2 是为了取排除以 / 结尾的路径）
			rootDirEnd = location.lastIndexOf('/', rootDirEnd - 2) + 1;
		}

		// 如果是 classpath*:*，经过上面循环得出 end 索引为 0，直接截取到 : 位置，也就是前缀
		if (rootDirEnd == 0) {
